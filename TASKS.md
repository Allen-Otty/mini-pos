...
- [x] Diagnosed the OTP `otp_expired` bug precisely via direct Supabase auth log inspection (not guessing): verify attempts failing 18–52 seconds after signup, `type: 'signup'` confirmed correct in code, user confirmed the dashboard's OTP expiry setting genuinely shows 3600s saved. Conclusion: Supabase-side config propagation issue, not fixable from our code or dashboard.
- [x] **Removed the OTP step entirely** per the owner's explicit decision (superseding the earlier "leave as-is for now"): rewrote `create-business` to create the auth user pre-confirmed via the Admin API (`email_confirm: true`, `verify_jwt: false` on the function since no session exists yet at signup) and create the business + profile in the same call. Signup is now a single step with immediate login, no email step at all.
...
## ✅ Completed (2026-09-01 session)

- [x] Fixed real bug: session persistence was on by default (`createClient` had no `auth` options) — Supabase silently saved sessions to localStorage, so anyone who'd logged in on a device stayed logged in on every future app launch, no credentials needed. Set `persistSession: false`; every launch now requires explicit login, matching the shared-terminal/shift model. Removed the now-dead `initSession()` restore-on-load IIFE.
- [x] Removed OTP entirely per owner's explicit decision (see note above) — deployed a rewritten `create-business` edge function (Admin API, `email_confirm: true`, `verify_jwt: false`), removed `handleVerifyOtp`/`handleResendOtp`/`showOtpPanel`/`pendingSignup` and the OTP panel markup, refactored `handleLogin` to extract a shared `performLogin()` used by both login and post-signup auto-login.
- [x] Fixed real bug: camera scanner auto-started on every login (`initScanner()` in `afterLogin()`) and ran for the entire session regardless of tab. Added a Start/Stop button (`toggleScanner()`), removed the auto-start call, and scanner now stops automatically on switching away from the Sell tab.
- [x] Fixed real bug: the open shift was found by `status === 'OPEN'` alone with no date scoping, so a shift opened yesterday and never closed would silently stay "current" today, carrying over its stale opening float and letting sales continue against a day-old shift. `currentShift` is now scoped to today's date only; a separate `staleOpenShift` is tracked and surfaced as a warning banner with a one-tap "close it now" action (`closeShift()` refactored to accept an optional shift param instead of only operating on the global). Opening float now defaults to a real `0.00` value (editable) each time the closed-shift view renders, instead of an empty field.
- [x] Checkout now blocked entirely without an open shift for today — guarded in `checkout()` itself (covers both the Sell-tab and table-order paths, which both funnel through it) plus the two button-animation wrappers (`handleCheckoutClick`/`handleTableCheckoutClick`), so the truck/"Ordered" success animation can't play when the sale was actually blocked.
